...
class CVInterface:
    ...
    def start_video_pipeline(self, video_path: str) -> bool:
        if self._vision_running:
            return True
        try:
            # main.py is normally launched from dashboard/, so add the
            # repository root before importing the sibling vision package.
            repo_root = Path(__file__).resolve().parents[2]
            if str(repo_root) not in sys.path:
                sys.path.insert(0, str(repo_root))
            from vision.vision_pipeline import run_perception_pipeline
        except Exception as exc:
            self._vision_error = f"Could not import vision pipeline: {exc}"
            return False
        ...
        self._vision_thread = threading.Thread(target=worker, name="CVVisionPipeline", daemon=True)
    ...
    def receive_vision_result(self, result: dict):
        ...
        state = self._STATE_MAP.get(result.get("state", "IDLE"), SystemState.SEARCHING)
    ...
    def get_fps(self) -> float:
        ...
        return round(fps, 1) if fps else round(self._fps_estimate, 1)
